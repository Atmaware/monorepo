...
const gqlRequest = async (query: string) => {
  const apiKey = (await getStorageItem('ruminerApiKey')) as string | undefined
  let headers = {
  ...
  try {
    ...
    console.log('[ruminer] error making api request: ', query)
  }
}
...
export async function savePageRequest(input: {
  ...
  if (data.savePage?.errorCodes?.length) {
    console.log('[ruminer] api: error saving page:', data)
    if (data.savePage.errorCodes.indexOf('UNAUTHORIZED') > -1) {
      console.log('[ruminer] api is not authorized')
...
export async function addNoteToLibraryItem(input: {
  ...
  if (data.article?.errorCodes?.length) {
    console.log('[ruminer] api: error getting article:', data)
