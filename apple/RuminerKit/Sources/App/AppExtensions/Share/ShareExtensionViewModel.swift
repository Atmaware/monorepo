...
public class ShareExtensionViewModel: ObservableObject {
  ...
  func handleReadNowAction(extensionContext: NSExtensionContext?) {
      ...
      if let application = UIApplication.value(forKeyPath: #keyPath(UIApplication.shared)) as? UIApplication {
        let deepLinkUrl = NSURL(string: "ruminer://shareExtensionRequestID/\(requestId)")
  ...
  func saveNote() {
    ...
    func queueSaveOperation(_ payload: PageScrapePayload) {
      ProcessInfo().performExpiringActivity(withReason: "app.ruminer.SaveActivity") { [self] expiring in
        guard !expiring else {
