...
public final class Services {
  static let fetchTaskID = "app.ruminer.fetchLinkedItems"
  static let secondsToWaitBeforeNextBackgroundRefresh: TimeInterval = isDebug ? 0 : 3600 // 1 hour
  static let logger = Logger(subsystem: "app.ruminer", category: "services-class")
    ...
    public static func registerBackgroundFetch() {
      BGTaskScheduler.shared.register(forTaskWithIdentifier: fetchTaskID, using: nil) { task in
        if let task = task as? BGAppRefreshTask {
          EventTracker.trackForDebugging("executing app.ruminer.fetchLinkedItems bg task")
...
// e -l objc -- (void)[[BGTaskScheduler sharedScheduler] _simulateLaunchForTaskWithIdentifier:@"app.ruminer.fetchLinkedItems"]
