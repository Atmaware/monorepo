...
@MainActor final class ProfileContainerViewModel: ObservableObject {
  ...
  var appVersionString: String {
    if let appVersion = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String {
      return "Ruminer Version \(appVersion)"
    } else {
      return ""
    }
  }
  ...
  private var innerBody: some View {
    Group {
      ...
      Section {
        Button(
          action: {
            if let url = URL(string: "https://docs.ruminer.app") {
              openURL(url)
            }
          },
      ...
      Section {
        Button(
          action: {
            if let url = URL(string: "https://ruminer.app/privacy") {
              openURL(url)
            }
          },
        ...
        Button(
          action: {
            if let url = URL(string: "https://ruminer.app/terms") {
              openURL(url)
            }
          },
...
extension BasicWebAppView {
  static func privacyPolicyWebView(baseURL: URL) -> BasicWebAppView {
    ruminerWebView(path: "/privacy", baseURL: baseURL)
  }

  static func termsConditionsWebView(baseURL: URL) -> BasicWebAppView {
    ruminerWebView(path: "/terms", baseURL: baseURL)
  }

  private static func ruminerWebView(path: String, baseURL: URL) -> BasicWebAppView {
    let url: URL = {
