...
struct WebReader: PlatformViewRepresentable {
  ...
  private func updatePlatformView(_ webView: WKWebView, context: Context) {
    if annotationSaveTransactionID != context.coordinator.lastSavedAnnotationID {
      ...
      do {
        try (webView as? RuminerWebView)?.dispatchEvent(.saveAnnotation(annotation: annotation))
      } catch {
    ...
    if readerSettingsChangedTransactionID != context.coordinator.previousReaderSettingsChangedUUID {
      ...
      (webView as? RuminerWebView)?.updateTheme()
      (webView as? RuminerWebView)?.updateFontFamily()
      (webView as? RuminerWebView)?.updateFontSize()
      (webView as? RuminerWebView)?.updateTextContrast()
      (webView as? RuminerWebView)?.updateAutoHighlightMode()
      (webView as? RuminerWebView)?.updateMaxWidthPercentage()
      (webView as? RuminerWebView)?.updateLineHeight()
      (webView as? RuminerWebView)?.updateLabels(labelsJSON: item.labelsJSONString)
      (webView as? RuminerWebView)?.updateTitle(title: item.title ?? "")
      (webView as? RuminerWebView)?.updateJustifyText()
    ...
    if shareActionID != context.coordinator.previousShareActionID {
      ...
      (webView as? RuminerWebView)?.shareOriginalItem()
    }
