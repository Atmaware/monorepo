...
struct SelfHostSettingsView: View {
  @State var apiServerAddress = UserDefaults(suiteName: "group.app.ruminerapp")?.string(forKey: AppEnvironmentUserDefaultKey.serverBaseURL.rawValue) ?? ""
  @State var webServerAddress = UserDefaults(suiteName: "group.app.ruminerapp")?.string(forKey: AppEnvironmentUserDefaultKey.webAppBaseURL.rawValue) ?? ""
  @State var ttsServerAddress = UserDefaults(suiteName: "group.app.ruminerapp")?.string(forKey: AppEnvironmentUserDefaultKey.ttsBaseURL.rawValue) ?? ""
  ...
  var innerBody: some View {
    ScrollView {
      VStack {
        ...
        TextField("URL", text: $apiServerAddress, prompt: Text("https://api-prod.ruminer.app"))
        ...
        TextField("URL", text: $webServerAddress, prompt: Text("https://ruminer.app"))
          .autocorrectionDisabled(true)
        #if os(iOS)
          .keyboardType(.URL)
          .textInputAutocapitalization(.never)
        ...
        TextField("URL", text: $ttsServerAddress, prompt: Text("https://tts.ruminer.app"))
          .autocorrectionDisabled(true)
        #if os(iOS)
          .keyboardType(.URL)
          .textInputAutocapitalization(.never)
        ...
        Text("""
        Ruminer is a free and open-source project and allows self-hosting.
        ...
        [Learn more about self-hosting Ruminer](https://docs.ruminer.app/self-hosting/self-hosting.html)
        """)
          .accentColor(.blue)
          .frame(maxWidth: .infinity, alignment: .leading)

        Button(action: {
          AppEnvironment.setCustom(serverBaseURL: "https://api-prod.ruminer.app",
                                   webAppBaseURL: "https://ruminer.app",
                                   ttsBaseURL: "https://tts.ruminer.app")
