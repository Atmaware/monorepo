...
enum ValetKey: String {
  case authToken = "app.ruminer.valet.auth-token"
  case authCookieString = "app.ruminer.valet.auth-cookie-raw-string"
  case appEnvironmentString = "app.ruminer.valet.app-environment"
  case hideLabels = "app.ruminer.valet.hide-labels"
}
...
extension ValetKey {
  ...
  private static let appGroupIdentifier: SharedGroupIdentifier = {
    #if os(macOS)
      return SharedGroupIdentifier(appIDPrefix: "QJF2XZ86HB", nonEmptyGroup: "app.ruminerapp.shared")!
    #endif

    #if os(iOS)
      return SharedGroupIdentifier(groupPrefix: "group", nonEmptyGroup: "app.ruminerapp")!
    #endif
  }()
}
