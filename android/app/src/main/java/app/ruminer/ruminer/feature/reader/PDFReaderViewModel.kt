...
import app.ruminer.ruminer.core.network.ReadingProgressParams
import app.ruminer.ruminer.core.network.createHighlight
import app.ruminer.ruminer.core.network.deleteHighlights
import app.ruminer.ruminer.core.network.mergeHighlights
import app.ruminer.ruminer.core.network.savedItem
import app.ruminer.ruminer.core.network.updateHighlight
import app.ruminer.ruminer.core.network.updateReadingProgress
import app.ruminer.ruminer.core.data.DataService
import app.ruminer.ruminer.core.data.NanoId
import app.ruminer.ruminer.graphql.generated.type.CreateHighlightInput
import app.ruminer.ruminer.graphql.generated.type.MergeHighlightInput
...
import app.ruminer.ruminer.core.network.getUriForInternalFile
...
//      handleDownloadError(exception)
  ...
  fun syncHighlightUpdates(newAnnotation: Annotation, quote: String, overlapIds: List<String>, note: String? = null) {
    ...
    newAnnotation.customData = JSONObject().put("ruminerHighlight", jsonValues)
  ...
  private fun storeUpdatedNoteLocally(annotation: Annotation, note: String) {
    val ruminerHighlight = annotation.customData?.get("ruminerHighlight") as? JSONObject
    ruminerHighlight?.put("editedNote", note)
    ruminerHighlight?.let {
      Log.d("pdf", "setting custom data: $ruminerHighlight")
      annotation.customData = JSONObject().put("ruminerHighlight", it)
    }
  }
  ...
  fun pluckHighlightID(annotation: Annotation): String? {
    val ruminerHighlight = annotation.customData?.get("ruminerHighlight") as? JSONObject
    return ruminerHighlight?.get("id") as? String
  }

  fun pluckExistingNote(annotation: Annotation): String? {
    val ruminerHighlight = annotation.customData?.opt("ruminerHighlight") as? JSONObject ?: return null

    val editedNote = ruminerHighlight.opt("editedNote") as? String
    if (editedNote != null) { return editedNote }

    val shortID = ruminerHighlight.get("shortId") as? String ?: return null
