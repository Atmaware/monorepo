...
const createRandomEmailAddress = (userName: string, length: number): string => {
  // format: hongbo-sduhfsjh1e@inbox.ruminer.app
  const inbox =
  ...
  when rand is sdfsdf-: jacksonh-sdfsdf-e@inbox.ruminer.app
  when rand is abcdef: jacksonh-abcdefe@inbox.ruminer.app
   */
  return `${userName}-${nanoid(length)}e@${
    env.email.domain || `@${inbox}.ruminer.app`
  }`
}
