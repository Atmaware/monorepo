...
const getPopularTopics = (
  ...
  return appDataSource.query(
    `
      ...
      FROM ruminer.discover_feed_articles 
      LEFT JOIN (SELECT discover_article_id as article_id, count(*) as count FROM ruminer.discover_feed_save_link group by discover_article_id) sl on id=sl.article_id
      LEFT JOIN (SELECT discover_article_id, article_save_id, article_save_url FROM ruminer.discover_feed_save_link WHERE user_id=$1 and deleted = false) su on id=su.discover_article_id
      WHERE COALESCE(sl.count / (EXTRACT(EPOCH FROM (NOW() - published_at)) / 3600 / 24), 0)  > 0.0
      AND (feed_id in (SELECT feed_id FROM ruminer.discover_feed_subscription WHERE user_id = $1) OR feed_id = '${COMMUNITY_FEED_ID}')       ${
...
const getAllTopics = (
  ...
  return appDataSource.query(
    `
      ...
      FROM ruminer.discover_feed_articles 
      LEFT JOIN (SELECT discover_article_id, article_save_id, article_save_url FROM ruminer.discover_feed_save_link WHERE user_id=$1 and deleted = false) su on id=su.discover_article_id
      WHERE (feed_id in (SELECT feed_id FROM ruminer.discover_feed_subscription WHERE user_id = $1) OR feed_id = '${COMMUNITY_FEED_ID}') 
      ${feedId != null ? `AND feed_id = $4` : ''}
...
const getTopicInformation = (
  ...
  return appDataSource.query(
    `SELECT id, title, feed_id as feed, slug, description, url, author, image, published_at, article_save_id, article_save_url 
     FROM ruminer.discover_feed_articles
     INNER JOIN (SELECT discover_feed_article_id FROM ruminer.discover_feed_article_topic_link WHERE discover_topic_name=$2) topic on topic.discover_feed_article_id=id
     LEFT JOIN (SELECT discover_article_id, article_save_id, article_save_url FROM ruminer.discover_feed_save_link WHERE user_id=$1 and deleted = false) su on id=su.discover_article_id
     WHERE (feed_id in (SELECT feed_id FROM ruminer.discover_feed_subscription WHERE user_id = $1) OR feed_id = '${COMMUNITY_FEED_ID}')  
...
>(async (_, { discoverTopicId, feedId, first, after }, { uid, log }) => {
  try {
    ...
    const { rows: topics } = (await appDataSource.query(
      `SELECT * FROM "ruminer"."discover_topics" WHERE "name" = $1`,
      [discoverTopicId]
    )) as { rows: unknown[] }
