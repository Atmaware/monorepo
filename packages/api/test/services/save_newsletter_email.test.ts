...
describe('saveNewsletterEmail', () => {
  ...
  it('adds the newsletter to the library', async () => {
    nock('https://blog.ruminer.app').get('/fake-url').reply(200)
    nock('https://blog.ruminer.app').head('/fake-url').reply(200)
    const url = 'https://blog.ruminer.app/fake-url'
    ...
    await saveNewsletter(
      {
        ...
        unsubHttpUrl: 'https://blog.ruminer.app/unsubscribe',
      },
      newsletterEmail
    )
  ...
  it('adds a Newsletter label to that page', async () => {
    nock('https://blog.ruminer.app').get('/new-fake-url').reply(200)
    nock('https://blog.ruminer.app').head('/new-fake-url').reply(200)
    const url = 'https://blog.ruminer.app/new-fake-url'
    const newLabel = {
      name: 'Newsletter',
      color: '#07D2D1',
    }
  ...
  it('does not create a subscription if no unsubscribe header', async () => {
    const url = 'https://ruminer.app/no_url?q=no-unsubscribe'
    nock('https://ruminer.app').get('/no_url?q=no-unsubscribe').reply(404)
